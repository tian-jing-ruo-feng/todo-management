...
export class Priority extends Entity<ToDoDb> {
  ...
  async makeSharable() {
    // Compute a deterministic realmId tied to this prioritys:
    const realmId = getTiedRealmId(this.id)

    const db = this.db // Entity<T> provides this.db - avoids cyclic deps.
    await db.transaction('rw', [db.priorities, db.tasks, db.realms], () => {
      ...
      // Move the prioritys into the new realm
      db.priorities.update(this.id!, { realmId: realmId })

      // Move all todo items into the new realm consistently
      // (modify() is consistent across sync peers)
      db.priorities.where({ id: this.id }).modify({ realmId: realmId })
    })
    return realmId
  }
}
